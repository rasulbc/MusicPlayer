...
public class MusicPlayer {
        ...
        while (option!="0"){
            ...
            switch (option){
                ...
                case "2": deleteUser();
                    break;
                case "3":
                    System.out.print("Please enter the Users ID:"); /* Here , I am taking UserId as an input , not username*/
                    id=input.nextInt();                             /*Because username is not unique in my app and if two users register with the same name*/
                    getUserDetails(id);                             /*It can cause some errors */
                    break;
                case "4": addSong();
                    break;
                case "5": deleteSong();
    ...
    public void deleteSong(){
        ...
        for(Song song:songs){
            if(id == song.getSongId()){
                found = 1;
                songs.remove(song);
                System.out.print("\nSongId "+id+" deleted successfully");
                break;
            }

        }
        if (found == 0 )  System.out.print("\nSong with that id was not found.");
    }
        ...
        A: for (User user: users){
            B: for (Playlist playlist: user.getListOfPlaylist()){
                if(playlist_id == playlist.getPlayListId()){
                    Scanner input = new Scanner(System.in);
                    System.out.print("\nEnter the Song ID to add to playlist:");
    ...
    public void displayUsers(){
        for(int i = 0; i<users.size();i++){
            System.out.print("\nUserId:  " + users.get(i).getUserId() + "   Username: " + users.get(i).getUserName());
        }
    };
    public void displaySongs(){
        for(int i = 0; i<songs.size();i++){
            System.out.print("\nSongId:  " + songs.get(i).getSongId() + "   Song Name: " + songs.get(i).getSongName());
        }
    };
